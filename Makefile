...
LDFLAGS := 
CPPFLAGS ?= $(INC_FLAGS) -Wall -pthread -MMD -MP -shared -fPIC -std=c++14 -O3 -mavx -ftree-vectorize -fopt-info-vec
...
CUDAFLAGS = $(INC_FLAGS) -v -g -lineinfo --expt-relaxed-constexpr -std=c++14 -O3 -DCUDA -DNOT_IMPL -arch=sm_70 -gencode=arch=compute_70,code=sm_70 -gencode=arch=compute_70,code=compute_70 -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_75,code=compute_75
