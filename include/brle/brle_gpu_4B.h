...
namespace brle {
	...
	__global__ void decompress_func(const uint8_t* const in, uint8_t* out,
	    ...
	    while(used_bytes < mychunk_size){
	    	...
	    	if(block == 0 || (c_flag == 0 && compress_flag == false)) {
				v = in[in_start_idx + used_bytes + in_off + tid * 4];
	    	...
	    	else if(compress_flag == true){

				for(uint8_t k = 0; k < compress_counter; k++){
						
					for(int i = 0 ; i < char_len; i++){
						out[out_start_idx + out_off + col_idx * 4] = v_buffer[i];
						out_off++;
						out_flag++;
						if(out_flag == 4){
