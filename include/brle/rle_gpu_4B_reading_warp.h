...
namespace brle_trans {
  ...
  decompress_func(const uint8_t* const in, uint8_t* out,
		...
		__shared__ uint32_t in_[SHARED_MEM_SIZE][32];
		...
		if (which == 0) {
			...
			while(used_bytes < mychunk_size){
				...
				r1:
				    ...
				    const auto next_tail = (cur_tail  + 1) % SHARED_MEM_SIZE;
			...
			while(used_bytes  < mychunk_size){
				...
				r2:
					...
					const auto next_head = (cur_head + 1) % SHARED_MEM_SIZE;
    ...
    *out_n_bytes = out_bytes;
    ...
    decompress_func<<<(num_blk), dim3(blk_size,2,1) >>> (d_in, d_out, num_blk, d_col_len, d_col_map, d_blk_offset);
