...
namespace brle_trans {
	...
	__global__ void col_count_gen(uint64_t* col_len, uint8_t* col_count, uint64_t* col_diff){
		...
		col_count[chunk_idx * 32 + threadIdx.x] = shared_col_count[threadIdx.x];
		col_diff[chunk_idx * 32 + threadIdx.x] = shared_col_diff[threadIdx.x];
  ...
  decompress_func(const uint8_t* const in, uint8_t* out,
    							    ...
    							    uint64_t* col_diff, uint8_t* col_count
    							    ){
        ...
        int chunk_idx = blockIdx.x;
	    shared_col_len[threadIdx.x] = col_len[BLK_SIZE * chunk_idx + tid];
	    shared_col_diff[threadIdx.x] = col_diff[BLK_SIZE * chunk_idx + tid];
	    shared_col_count[threadIdx.x] = col_count[BLK_SIZE * chunk_idx + tid];
		...
		//uint64_t temp_count = shared_col_diff[track_idx + 32 * chunk_idx];
		uint64_t temp_count = shared_col_diff[track_idx + track_start];
		...
		while(used_bytes  < mychunk_size){
			...
			if( num_read_count != 0 && (input_buffer_read_count < mychunk_size)){
				for(int i = 0; i < num_read_count; i++){
					...
					if(temp_count == 0){
						if(track_idx < 32){
							//uint64_t count_shift = shared_col_count[track_idx + 32 * chunk_idx];
							uint64_t count_shift = shared_col_count[track_idx + track_start];

							col_counter -= count_shift;
							track_idx += count_shift;
							//temp_count = shared_col_diff[track_idx + 32 * chunk_idx];
							temp_count = shared_col_diff[track_idx + track_start];
    ...
    *out_n_bytes = out_bytes;
	...
	cuda_err_chk(cudaMalloc(&d_col_diff, sizeof(uint64_t)*32*num_blk));
	cuda_err_chk(cudaMalloc(&d_col_count, sizeof(uint8_t)*32*num_blk));

	col_count_gen<<<num_blk, 32>>>(d_col_len, d_col_count, d_col_diff);
	cudaDeviceSynchronize();

    //decompress_func<<<(num_blk/4), blk_size * 4>>> (d_in, d_out, num_blk, d_col_len, d_col_map, d_blk_offset);
    decompress_func<<<(num_blk), blk_size >>> (d_in, d_out, num_blk, d_col_len, d_col_map, d_blk_offset, d_col_diff, d_col_count);
   // decompress_func<<<(num_blk/4), blk_size*4 >>> (d_in, d_out, num_blk, d_col_len, d_col_map, d_blk_offset, d_col_diff, d_col_count);
