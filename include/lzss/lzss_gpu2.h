...
>>>>>>> 2521a77ef53923581b24ccb056259d35bc9e5a2d
    	...
    	while(used_bytes < mychunk_size){
			...
			while ((lookahead_count < LOOKAHEAD_SIZE) && (consumed_bytes < mychunk_size))  {
				 lookahead[(lookahead_head + (lookahead_count++)) % LOOKAHEAD_SIZE] = in[in_start_idx + (consumed_bytes++) + in_off + tid * 4];
		...
		//match
			...
			// if(tid + chunk_idx == 0){
			// 	printf("LENGTH_SIZE: %i MIN_MATCH_LENGTH: %i\n", LENGTH_SIZE, MIN_MATCH_LENGTH);
			// }
		...
		col_len[BLK_SIZE*chunk_idx + tid] = out_bytes; 


		uint64_t out_len_4B = ((out_bytes + 3)/4)*4;
...
__global__ void compress_func_new (const uint8_t* const in, uint8_t* out, const uint64_t in_n_bytes, uint64_t* out_n_bytes, const uint64_t in_chunk_size, 
		...
		while(used_bytes < mychunk_size){
			...
			while ((lookahead_count < LOOKAHEAD_SIZE) && (consumed_bytes < mychunk_size))  {
				 uint8_t read_data = in[in_start_idx + (consumed_bytes++) + in_off + tid * 4];
				 lookahead[(lookahead_head + (lookahead_count++)) % LOOKAHEAD_SIZE] = read_data;
...
>>>>>>> 2521a77ef53923581b24ccb056259d35bc9e5a2d
	...
	*out = new uint8_t[final_out_size];
	...
	compress_func_new<<<n_chunks, BLK_SIZE>>>(d_in, d_out, in_n_bytes, out_n_bytes, chunk_size, n_chunks,
...
<<<<<<< HEAD
	...
	uint64_t* blk_off;
	if((blk_off_fd = open("./input_data/blk_offset.bin", O_RDONLY)) == 0) {
	...
	uint64_t* col_len;
	if((col_len_fd = open("./input_data/col_len.bin", O_RDONLY)) == 0) {
	...
	uint8_t* col_map;
	if((col_map_fd = open("./input_data/col_map.bin", O_RDONLY)) == 0) {
